...
class Sam3VideoInference(Sam3VideoBase):
    ...
    def _run_single_frame_inference(self, inference_state, frame_idx, reverse):
        ...
        if self.rank == 0:
            keep_history_len = 10 
            oldest_allowed_idx = frame_idx - keep_history_len

            if oldest_allowed_idx >= 0:
                if "tracker_inference_states" in inference_state:
                    tis = inference_state["tracker_inference_states"]
                    
                    if isinstance(tis, list):
                        for tracker_state in tis:
                            if not isinstance(tracker_state, dict): continue

                            if "output_dict" in tracker_state:
                                output_dict_internal = tracker_state["output_dict"]
                                if "non_cond_frame_outputs" in output_dict_internal:
                                    #print("m1")
                                    storage = output_dict_internal["non_cond_frame_outputs"]
                                    keys_to_remove = [k for k in storage.keys() 
                                                      if isinstance(k, int) and k < oldest_allowed_idx and k != 0]
                                    for k in keys_to_remove: del storage[k]

                            if "output_dict_per_obj" in tracker_state:
                                odpo = tracker_state["output_dict_per_obj"]
                                for obj_id in odpo:
                                    obj_data = odpo[obj_id]
                                    if "non_cond_frame_outputs" in obj_data:
                                        #print("m2")
                                        
                                        storage = obj_data["non_cond_frame_outputs"]
                                        keys_to_remove = [k for k in storage.keys() 
                                                          if isinstance(k, int) and k < oldest_allowed_idx and k != 0]
                                        for k in keys_to_remove: del storage[k]
                            if "temp_output_dict_per_obj" in tracker_state:
                                todpo = tracker_state["temp_output_dict_per_obj"]
                                for obj_id in todpo:
                                    obj_data = todpo[obj_id]
                                    if "non_cond_frame_outputs" in obj_data:
                                        #print("m3")
                                        storage = obj_data["non_cond_frame_outputs"]
                                        keys_to_remove = [k for k in storage.keys() 
                            ...
                            if "cached_features" in tracker_state:
                                #print("m4")
                                cf = tracker_state["cached_features"]
                                if "multigpu_buffer" in cf:
                                    
                                    buffer = cf["multigpu_buffer"]
                                    keys_to_remove = [k for k in buffer.keys() 
                                                      if isinstance(k, int) and k < oldest_allowed_idx]
                                    for k in keys_to_remove: del buffer[k]
                ...
                if "feature_cache" in inference_state:
                    ...
                    keys_to_remove = [k for k in cache.keys() if isinstance(k, int) and k < oldest_allowed_idx]
                    for k in keys_to_remove:
                        del cache[k]
                if "cached_frame_outputs" in inference_state:
                    #print("m6")
                    cache_out = inference_state["cached_frame_outputs"]
                    keys_to_remove = [k for k in cache_out.keys() if isinstance(k, int) and k < oldest_allowed_idx]
                    for k in keys_to_remove:
                        del cache_out[k]
            ...
            if frame_idx % 50 == 0:
                ...
                print(f"[Frame {frame_idx}] GPU Memory: Allocated {allocated:.2f} GB | Reserved {reserved:.2f} GB")
