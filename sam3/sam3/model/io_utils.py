...
class AsyncVideoLoader:
    ...
    def __init__(self, video_path, image_size, offload_video_to_cpu, img_mean, img_std):
        ...
        self._vcap = cv2.VideoCapture(video_path)
        if not self._vcap.isOpened():
            raise ValueError(f"Could not open video: {video_path}")

        self.orig_w = int(self._vcap.get(cv2.CAP_PROP_FRAME_WIDTH))
        self.orig_h = int(self._vcap.get(cv2.CAP_PROP_FRAME_HEIGHT))
        self._num_frames = int(self._vcap.get(cv2.CAP_PROP_FRAME_COUNT))
        self._next_fidx = 0

        self._offload_video_to_cpu = offload_video_to_cpu
        self._img_mean = torch.tensor(img_mean).view(3, 1, 1)
        self._img_std = torch.tensor(img_std).view(3, 1, 1)
        self._img_wh = (image_size, image_size)

    def __len__(self):
        return self._num_frames
    def __getitem__(self, index):

        # Only jump the video if we're not on the right frame already
        if index != self._next_fidx:
            self._vcap.set(cv2.CAP_PROP_POS_FRAMES, index)
        ok_frame, frame = self._vcap.read()
        self._next_fidx = index + 1

        # Make a 'fake' empty frame if we can't read, so we don't raise an error
        # (might happen if the video is missing frames, especially at the end)
        if not ok_frame:
            frame = np.zeros((self.orig_h, self.orig_w, 3), dtype=np.uint8)

        # Process frame into normalized tensor format
        frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        frame_rgb = cv2.resize(frame_rgb, self._img_wh, interpolation=cv2.INTER_CUBIC)
        frame_tensor = torch.from_numpy(frame_rgb).permute(2, 0, 1)  # (C, H, W)
        frame_tensor = frame_tensor.to(dtype=torch.float16)
        frame_tensor -= self._img_mean
        frame_tensor /= self._img_std
        if not self._offload_video_to_cpu:
...
def load_video_frames_from_video_file_using_cv2(
    ...
    video_loader = AsyncVideoLoader(
        video_path, image_size, offload_video_to_cpu, img_mean, img_std
    )
    print("Using async")
    return video_loader, video_loader.orig_h, video_loader.orig_w


def load_video_frames_from_video_file_using_cv2_original(
    video_path: str,
    image_size: int,
    img_mean: tuple = (0.5, 0.5, 0.5),
    img_std: tuple = (0.5, 0.5, 0.5),
